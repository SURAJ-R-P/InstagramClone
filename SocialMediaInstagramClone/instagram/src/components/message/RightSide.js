...
const RightSide = () => {
    ...
    // Call
    const caller = ({video}) => {
        const { _id, avatar, user_name, full_name } = user

        const msg = {
            sender: auth.user._id,
            recipient: _id, 
            avatar, user_name, full_name, video
        }
        dispatch({ type: GLOBALTYPES.CALL, payload: msg })
    }

    const callUser = ({video}) => {
        const { _id, avatar, user_name, full_name } = auth.user
        const msg = {
            sender: _id,
            recipient: user._id, 
            avatar, user_name, full_name, video
            ...
            style={{height: media.length > 0 ? 'calc(100% - 180px)' : ''}} >
                <div className="chat_display" ref={refDisplay}>
                    ...
                    }
                    {
                       loadMedia && 
                       <div className="chat_row your_message">
                           <img src={LoadIcon} alt="loading" className="d-block" style={{borderRadius:"100%",width:"50px",height:"50px"}}/>
                       </div>
                    }
                </div>
            </div>
